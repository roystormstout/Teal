...
function detailTask(event) {

    var tar_description = "An upcoming appointment for Allen."
    // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
    // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
    var modal = $("#exampleModal");
    modal.find('.modal-title').text("Upcoming Session: Appointment")
    modal.find('#session-date').text("Time: 1pm")
    modal.find('#session-description').text("Description: "+ tar_description)
